...
telescope.setup {
  ...
  extensions = {
    file_browser = {
      theme = 'dropdown',
      -- disable netrw add use telescope_file_browser in its place
      hijack_netrw = true,
      mappings = {
        ['i'] = {
...
vim.keymap.set('n', ';e', '<cmd>lua require("telescope.builtin").diagnostics()<cr>',
  opts)
vim.keymap.set('n', 'sf', '<cmd>lua require("telescope").extensions.file_browser.file_browser({ path = "%:p:h", cwd = telescope_buffer_dir(), respect_git_ignore = false, hidden = true, grouped = true, previewer = false, inital_mode = "normal", layout_config = { height = 40 }})<cr>',
  opts)
